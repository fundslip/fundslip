...
// Icons for connectors that don't self-report via EIP-6963
...
export function WalletOptions({ layout = "dropdown", onConnected }: WalletOptionsProps) {
  ...
  // Split: installed browser wallets (EIP-6963) vs protocol connectors (WC, Coinbase)
  const { installed, others } = useMemo(() => {
    ...
    for (const c of connectors) {
      if (c.id === "injected") continue; // hide generic fallback
  ...
  const handleSelect = useCallback(async (connector: (typeof connectors)[number]) => {
    ...
    if (connector.id === "walletConnect") {
      ...
      try {
        ...
        // connectAsync returns a promise — don't await, let QR show while waiting
      ...
      <div className={isCompact ? "space-y-0.5" : "space-y-2"}>
        {allWallets.map((connector, i) => (
          <div key={connector.uid}>
            {/* Separator between installed and protocol wallets */}
